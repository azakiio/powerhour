...
export default function LayoutAnimations() {
  return (
    <div className="slide">
      ...
      <p>
        Animating a style like <code>height</code> between <code>100px</code>{" "}
        and <code>500px</code> is conceptually straightforward, but suffers from
        poor performance because we're triggering the browser layout systems
        every animation frame.
      </p>
      <p>
        Sometimes it doesn't even make sense. What does it actually mean to
        animate <code>justify-content</code>
        between <code>flex-start</code> and <code>flex-end</code>? Framer Motion
        can animate between any CSS layout by using performant transforms
        instead of the layout system.
      </p>
      <p>
        To enable Framer Motion's layout animations, we simply set the layout
        prop of a motion component.
      </p>
      ...
      <ul>
        ...
        <li>
          A style set on the component itself, for example a change in width or
          position.{" "}
        </li>
